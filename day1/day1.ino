 ...
 *
 * @param low Container whose values are summed.
 * @param high Container whose values are summed.
...
uint32_t pivot_track_low(int16_t *low, int16_t *high, int16_t *pivot)
{
	...
	if (pivot >= high)
	{
		return (uint32_t)(high - 1);
	}
	else if (remainder < *pivot)
	{
		return pivot_track_low(low + 1, high, low + 2);
	}
	return (((uint32_t)low) << 16) | (uint32_t)pivot;
}

uint32_t pivot_track_high(int16_t *low, int16_t *high, int16_t *pivot)
{
	int16_t remainder = (2020 - (*low + *high));
	...
	if (pivot <= low)
	{
		return (uint32_t)(low + 1);
	}
	else if (remainder > *pivot)
	{
		return pivot_track_high(low, high - 1, high - 2);
	}
	return (((uint32_t)high) << 16) | (uint32_t)pivot;
}
...
uint32_t find_2020_triple(int16_t *low, int16_t *high)
{
	...
	for (;;)
	{
		...
		if (remainder > *pivot)
		{
			uint32_t result = pivot_track_low(low, high, pivot);

			if (result & 0xFFFF0000)
			{
				return uint32_t(*high) * *(int16_t *)((result & 0xFFFF0000) >> 16) * *(int16_t *)(result & 0xFFFF);
			}
			pivot = (int16_t *)(result & 0xFFFF);
			low++;
		}
		else if (remainder < *pivot)
		{
			uint32_t result = pivot_track_high(low, high, pivot);

			if (result & 0xFFFF0000)
			{
				return uint32_t(*low) * *(int16_t *)((result & 0xFFFF0000) >> 16) * *(int16_t *)(result & 0xFFFF);
			}
			pivot = (int16_t *)(result & 0xFFFF);
			high--;
		}
...
password_data read_integer(const char password[], uint8_t start)
{
	char read;
	password_data data;
	data.value = 0;
	do
	{
		...
		if (data.value > 0)
			data.value *= 10;
		data.value += (read - 48);
	} while (read > 47 && read < 58);
	data.index = start;
	delete password;
	return data;
}

uint8_t validate_password(const char password[])
{
	uint8_t min, max, index, count = 0;
	char compareValue, testValue;
	password_data data;

	data = read_integer(password, 0);
	min = data.value;

	data = read_integer(password, data.index);
	max = data.value;

	index = data.index;

	compareValue = password[index++];	

	do {
		testValue = password[index++];
...
void loop()
{
	if (!hasRun)
	{
		...
		uint8_t listSize = (sizeof(list) / sizeof(*list));

		startTime = micros();
		quick_sort(list, &list[listSize - 1]);
		endTime = micros();
		...
		startTime = micros();
		find_2020(list, &list[listSize - 1]);
		endTime = micros();
		duration = endTime - startTime;
		...
		startTime = micros();
		find_2020_triple(list, &list[listSize - 1]);
		endTime = micros();
		duration = endTime - startTime;
		Serial.print("find_2020_triple time: ");
		Serial.println(duration);

		uint8_t passwordSize = (sizeof(PASSWORDS) / sizeof(*PASSWORDS));

		startTime = micros();
		uint16_t validCount = passwords_validator(PASSWORDS, passwordSize);
		endTime = micros();
		duration = endTime - startTime;
		Serial.print("validate_passwords time: ");
		Serial.println(duration);
		Serial.print("validate_passwords value: ");
